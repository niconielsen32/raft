...
class RAFTStereo(nn.Module):
    def __init__(self, restore_ckpt, save_numpy, mixed_precision, valid_iters, hidden_dims, corr_implementation, shared_backbone, corr_levels, corr_radius, n_downsample, context_norm, slow_fast_gru, n_gru_layers):
        ...
        self.cnet = MultiBasicEncoder(output_dim=[self.hidden_dims, context_dims], norm_fn=self.context_norm, downsample=self.n_downsample)
        self.update_block = BasicMultiUpdateBlock(restore_ckpt, save_numpy, mixed_precision, valid_iters, hidden_dims, corr_implementation, shared_backbone, corr_levels, corr_radius, n_downsample, context_norm, slow_fast_gru, n_gru_layers)

        self.context_zqr_convs = nn.ModuleList([nn.Conv2d(context_dims[i], self.hidden_dims[i]*3, 3, padding=3//2) for i in range(self.n_gru_layers)])
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.mixed_precision):
            if self.shared_backbone:
                *cnet_list, x = self.cnet(torch.cat((image1, image2), dim=0), dual_inp=True, num_layers=2)
                fmap1, fmap2 = self.conv2(x).split(dim=0, split_size=x.shape[0]//2)
            else:
                cnet_list = self.cnet(image1, num_layers=2)
